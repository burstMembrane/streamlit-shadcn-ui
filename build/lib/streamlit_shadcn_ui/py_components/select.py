...
def select(
    ...
    key: Optional[str] = "ui_select",
):
    """
    Create a select component.

    Args:
        label (str, optional): The label of the select component. Defaults to None.
        options (List[str], optional): The options of the select component. Defaults to None.
        key (str, optional): The key of the select component. Defaults to "ui_select".
    ...
    init_session(
        key=options_component_key,
        default_value={"value": option_list[0], "open": False},
    )
    ...
    with stylable_container(
        ...
        trigger_state = select_trigger(
            value=choice, open_status=open_status, key=trigger_component_key
        )

        options_state = select_options(
            options=option_list,
            x=trigger_state["x"],
            y=trigger_state["y"],
            open_status=open_status,
            default_value=option_list[0],
            key=options_component_key,
            on_change=option_choosen_handler,
            kwargs={"from_key": options_component_key, "to_key": trigger_component_key},
        )
