...
with open(Path(__file__).parent.parent / "docs/components/separator.md", "r") as f:
    st.markdown(f.read())
...
with ui.element("div", key="base_ele_div_s1", className="space-y-1"):
    ...
    with ui.element(
        ...
        ui.element(
            "separator", key="separator_1", orientation="vertical", className="h-5"
        )
        ui.element("div", key="base_ele_div_s4", children="Docs")
        ui.element(
            "separator", key="separator_2", orientation="vertical", className="h-5"
        )
