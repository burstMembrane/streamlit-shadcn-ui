...
def date_picker_content(
    ...
    with container:
        ...
        result = _component_func(
            ...
            props=props,
            key=key,
            default={"value": default_value, "open": False},
        )

        return result


def date_chosen_handler(from_key, to_key):
...
def date_picker(
    ...
    Args:
        label (str, optional): The label of the date picker. Defaults to None.
        mode (str, optional): The mode of the date picker. Defaults to "single".
        default_value (str, optional): The default value of the date picker. Defaults to None.
        key (str, optional): The key of the date picker. Defaults to None.

    Examples:
    ```python
    import streamlit_shadcn_ui as ui
    # create a single date picker
    ui.date_picker(label="Select a date", mode="single", default_value=None, key="date_picker_1")


    # create a range date picker
    ui.date_picker(key="date_picker2", mode="range", label="Date Picker")
    ...
    init_session(
        key=content_component_key, default_value={"value": default_value, "open": False}
    )
    ...
    with stylable_container(
        ...
        trigger_pos = date_picker_trigger(
            value=value, label=label, open_status=open_status, key=trigger_component_key
        )
        # need to sync value, or "cancel" will not work
        st.session_state[content_component_key]["open"] = trigger_pos["open"]
        content_state = date_picker_content(
            value=value,
            mode=mode,
            x=trigger_pos["x"],
            y=trigger_pos["y"],
            ...
            on_change=date_chosen_handler,
            kwargs={"from_key": content_component_key, "to_key": trigger_component_key},
        )
